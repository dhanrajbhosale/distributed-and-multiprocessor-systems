# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

import example_pb2 as example__pb2


class RPCStub(object):
    """message Event {
    string interface = 1;
    int32 money = 2;
    string result = 3;
    int32 id = 4;
    int32 clock = 5;
    }

    service RPC {
    rpc MsgDelivery(Event) returns (Event) {}
    }

    """

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.MsgDelivery = channel.unary_unary(
                '/RPC/MsgDelivery',
                request_serializer=example__pb2.BranchRequest.SerializeToString,
                response_deserializer=example__pb2.BranchResponse.FromString,
                )


class RPCServicer(object):
    """message Event {
    string interface = 1;
    int32 money = 2;
    string result = 3;
    int32 id = 4;
    int32 clock = 5;
    }

    service RPC {
    rpc MsgDelivery(Event) returns (Event) {}
    }

    """

    def MsgDelivery(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_RPCServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'MsgDelivery': grpc.unary_unary_rpc_method_handler(
                    servicer.MsgDelivery,
                    request_deserializer=example__pb2.BranchRequest.FromString,
                    response_serializer=example__pb2.BranchResponse.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'RPC', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class RPC(object):
    """message Event {
    string interface = 1;
    int32 money = 2;
    string result = 3;
    int32 id = 4;
    int32 clock = 5;
    }

    service RPC {
    rpc MsgDelivery(Event) returns (Event) {}
    }

    """

    @staticmethod
    def MsgDelivery(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/RPC/MsgDelivery',
            example__pb2.BranchRequest.SerializeToString,
            example__pb2.BranchResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
